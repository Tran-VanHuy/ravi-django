...
class LandingPage(TemplateView):
    ...
    def get_context_data(self, **kwargs: Any) -> dict[str, Any]:
        ...
        banners = Banner.objects.filter(page=1)
        about_me = AboutMe.objects.all().order_by("-id").first()
        actions = Action.objects.all().order_by("-id").first()
        item_actions = ItemAction.objects.select_related("action").filter(action__id=actions.id)[:3]
        project = Project.objects.all().order_by("-id").first()
        item_projects = ItemProject.objects.filter(item__id=project.id, activity=True)
        ...
        item_partner = ItemPartner.objects.filter(item__id=partner.id)
...
class ProjectListPage(TemplateView):
    ...
    def get_context_data(self, **kwargs):
        ...
        projects = ItemProject.objects.all().order_by("-id")
